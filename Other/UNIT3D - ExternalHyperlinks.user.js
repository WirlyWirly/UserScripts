// ==UserScript==
// @name        UNIT3D - ExternalHyperlinks
// @author      WirlyWirly
// @version     1.1
// @description Add additional search links alongside those already present on a UNIT3D tracker
// @namespace   https://github.com/WirlyWirly
// @icon        https://raw.githubusercontent.com/WirlyWirly/UserScripts/main/monkey.png?raw=true
// @grant        none

// @include     /http?s://.*/torrents/\d+.*/
// @include     /http?s://.*/torrents/similar/[\d\.]+.*/

// @homepage    https://github.com/WirlyWirly/UserScripts/blob/main/Other/UNIT3D%20-%20ExternalHyperlinks.user.js
// @updateURL   https://raw.githubusercontent.com/WirlyWirly/UserScripts/main/Other/UNIT3D%20-%20ExternalHyperlinks.user.js?raw=true
// @downloadURL https://raw.githubusercontent.com/WirlyWirly/UserScripts/main/Other/UNIT3D%20-%20ExternalHyperlinks.user.js?raw=true

// ==/UserScript==

let externalLinksList, imdbId, mediaTitle, mediaType, abortScript = true

try {
    externalLinksList = document.querySelector('ul.meta__ids') // The unordered list element that contains a <li> for each external link, both the defaults and where the new ones will be appended
    imdbId = externalLinksList.querySelector('li.meta__imdb > a').href.match(/\/(tt\d+)/)[1] // The IMDB id, used to generate hyperlinks
    mediaTitle = encodeURIComponent(document.querySelector('h1.meta__title').innerText.match(/(.*) \(\d+\)/)[1]) // The title, used to generate hyperlinks that don't accept an IMDB id number
    mediaType = document.querySelector('ul.work__tags > li.work__media-type').innerText.toLowerCase().match(/(movie|tv)/)[1] // The mediaType, which affects which hyperlinks will be displayed
    abortScript = false

} catch(error) {
    console.info(`---------- 🐵 ExternalHyperlinks 🐵 ----------\n\nThis UNIT3D page does not have the necessary metadata or page layout to generate hyperlinks.`)
    // console.log(error)
}

if ( abortScript == false ) {
    // The metadata needed to generate external links was found, so continue with the script execution

    let hyperlinkTemplates = [
        // An array of objects, each object containing the required info to generate and present a external link

        {
            name: 'PassThePopcorn',
            types: ['movie'],
            searchURL: `https://passthepopcorn.me/torrents.php?action=advanced&searchstr=${imdbId}`,
            base64: 'data:image/x-icon;base64,',
        },

        {
            name: 'BroadcasTheNet',
            types: ['tv'],
            searchURL: `https://broadcasthe.net/torrents.php?action=advanced&imdb=${imdbId}`,
            base64: 'data:image/png;base64,',
        },

        {
            name: 'AnimeBytes',
            types: ['tv', 'movie'],
            searchURL: `https://animebytes.tv/torrents.php?searchstr=${mediaTitle}&search_type=title`,
            base64: 'data:image/png;base64,',
        },

        {
            name: 'HDBits',
            types: ['tv', 'movie'],
            searchURL: `https://hdbits.org/film/info?id=${imdbId.match(/(\d+)/)[1]}`,
            base64: 'data:image/png;base64,',
        },

        {
            name: 'Simkl',
            types: ['tv', 'movie'],
            searchURL: `https://api.simkl.com/redirect?to=Simkl&imdb=${imdbId}`,
            base64: 'data:image/png;base64,',
        },

        {
            name: 'Trakt',
            types: ['tv', 'movie'],
            searchURL: `https://trakt.tv/search/imdb/${imdbId}`,
            base64: 'data:image/png;base64,',
        },

        {
            name: 'TVDB',
            types: ['tv', 'movie'],
            searchURL: `https://thetvdb.com/redirect/imdb/${imdbId}`,
            base64: 'data:image/png;base64,',
        },
    ]

    for ( let template of hyperlinkTemplates ) {
        // Using the data in each template, generate a working hyperlink and add it to the page

        if ( template.types.includes(mediaType) ) {
            // The mediaType of the current torrent is one for which this externalLink is relevant

            // Create and append the elements that will contain the new search link
            let listElement = document.createElement('li')
            let hyperlinkElement = document.createElement('a')
            let imageElement = document.createElement('img')

            listElement.appendChild(hyperlinkElement)
            hyperlinkElement.appendChild(imageElement)
            externalLinksList.appendChild(listElement)

            // Populate the <li> and child elements with the required data
            listElement.classList.add('meta__externalLink')

            hyperlinkElement.classList.add('meta-id-tag')
            hyperlinkElement.href = template.searchURL
            hyperlinkElement.target = '_blank'
            hyperlinkElement.title = template.name

            imageElement.src = template.base64

        }

    }

}
