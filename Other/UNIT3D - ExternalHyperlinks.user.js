...
let externalLinksList, imdbId, mediaTitle, mediaType

try {
    externalLinksList = document.querySelector('ul.meta__ids') // The element that contains a <li> for each external link, both the defaults and the new ones
    imdbId = externalLinksList.querySelector('li.meta__imdb > a').href.match(/\/(tt\d+)/)[1] // The IMDB id, used by externalLinks
    mediaTitle = encodeURIComponent(document.querySelector('h1.meta__title').innerText.match(/(.*) \(\d+\)/)[1]) // The title, used by externalLinks that don't accept an IMDB id number
    mediaType = document.querySelector('ul.work__tags > li.work__media-type').innerText.toLowerCase().match(/(movie|tv)/)[1] // The mediaType, which affects which externalLinks will be displayed

} catch(error) {
    throw new Error('ExternalLinks: Unable to parse the data needed for generating external links, aborting further script execution')
}
...
for ( let external of externalLinks ) {
    ...
    if ( external.types.includes(mediaType) ) {
